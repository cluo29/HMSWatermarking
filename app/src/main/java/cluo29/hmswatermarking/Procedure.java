...
public class Procedure extends Service implements SensorEventListener {
    ...
    private static List<Integer> data_to_embed = new ArrayList<Integer>();
    ...
    public void onCreate() {
        ...
        label="acc "+SENSOR_DELAY + " heart "+Embed_DELAY;
    ...
    public void onSensorChanged(SensorEvent event) {
        ...
        if (sensor.getType() == Sensor.TYPE_ACCELEROMETER&&(!ready||!data_to_embed.isEmpty())) //!ready means watermarking ongoing for one reading.
        {
            ...
            if(ready){
                ...
                int heart_rate = data_to_embed.get(0);

                data_to_embed.remove(0);


                binaryData_x = Encryption(heart_rate);



                float result_watermarking = IntWatermarking(binaryData_x, accelerometer_x, BITS, embeddingComplete);
            ...
            else
            {
                ...
                if(embeddingComplete == bitsOfContainer)
                {
                    ...
                    //Log.d("SENSORS11", " watered readings = " + water_count);
                }
            ...
            //Log.d("SENSORS11", "processing time = "+ (System.currentTimeMillis() - startProcessingTime));
    ...
    public class ContextReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            {
                int heart_rate = intent.getExtras().getInt(EXTRA_DATA);

                data_to_embed.add(heart_rate);
    ...
    private static String IntToBinaryString(int data){ //int data to put in containers, 32-bit
        return String.format("%32s", Integer.toBinaryString(data)).replace(' ', '0');
    }

    //do watermarking for integers, container is float, accelerometer
    private static float IntWatermarking(String binaryData,  //binary data of int to put in containers
    ...
    public static String Encryption(int Data) { //binaryData for encryption

        String encrypted_binary_string="";



        byte[] input_string = ByteBuffer.allocate(4).putInt(Data).array();
        ...
        try {
            ...
            else if(encrypted_bits_counter == 32){ //after 32, next int to do

                key_index = 4;

            }
            else if(encrypted_bits_counter == 64){ //after 32*2, next int to do

                key_index = 8;
            }
            else if(encrypted_bits_counter == 96){ //after 32*3, next int to do
