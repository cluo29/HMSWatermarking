...
public class ThreeFloatComparison2Bits extends Service implements SensorEventListener {
    ...
    //original signals
    private static List<Float> originalsX = new ArrayList<Float>();

    //marked signals
    private static List<Float> markedX = new ArrayList<Float>();


    //original signals
    private static List<Float> originalsY = new ArrayList<Float>();

    //marked signals
    private static List<Float> markedY = new ArrayList<Float>();


    //original signals
    private static List<Float> originalsZ = new ArrayList<Float>();

    //marked signals
    private static List<Float> markedZ = new ArrayList<Float>();
    ...
    public void onSensorChanged(SensorEvent event) {
        ...
        public static final String LABEL  = "label";    //
                */
                ...
                for (int i =0; i<markedX.size();i++)
                {

                    ContentValues data = new ContentValues();
                    data.put(Provider.Unlock_Monitor_Data2.TIMESTAMP, -1*(i+1));
                    data.put(Provider.Unlock_Monitor_Data2.ORIGINALX, originalsX.get(i));
                    data.put(Provider.Unlock_Monitor_Data2.ORIGINALY, originalsY.get(i));
                    data.put(Provider.Unlock_Monitor_Data2.ORIGINALZ, originalsZ.get(i));
                    data.put(Provider.Unlock_Monitor_Data2.MARKEDX, markedX.get(i));
                    data.put(Provider.Unlock_Monitor_Data2.MARKEDY, markedY.get(i));
                    data.put(Provider.Unlock_Monitor_Data2.MARKEDZ, markedZ.get(i));
                    getContentResolver().insert(Provider.Unlock_Monitor_Data2.CONTENT_URI, data);
            ...
            //signal with/out watermarks to compare. Using DB

            float accelerometer_x = event.values[0];
            float accelerometer_y = event.values[1];
            float accelerometer_z = event.values[2];

            originalsX.add(accelerometer_x);
            originalsY.add(accelerometer_y);
            originalsZ.add(accelerometer_z);
            ...
            if(ready){
                ...
                //Log.d("SENSORS11", "diff=" +(result_watermarking_x-accelerometer_x));
                ...
                markedX.add(result_watermarking_x);
                markedY.add(result_watermarking_y);
                markedZ.add(result_watermarking_z);
