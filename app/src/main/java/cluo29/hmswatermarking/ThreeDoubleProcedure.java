...
public class ThreeDoubleProcedure extends Service implements SensorEventListener {
    ...
    //64 bits double
    private static int bitsOfContainer = 64;

    //how is the watering progress? 0 means nothing done, 64 means done for one reading
    ...
    public void onCreate() {
        ...
        label="acc "+SENSOR_DELAY + " GPS "+Embed_DELAY;
    ...
    public void onSensorChanged(SensorEvent event) {
        ...
        public static final String LABEL  = "label";    //
            ...
            if(ready){
                ...
                binaryData_x = Encryption(data.getAsDouble("double_x"));

                binaryData_y = Encryption(data.getAsDouble("double_y"));

                binaryData_z = Encryption(data.getAsDouble("double_z"));

                float result_watermarking_x = DoubleWatermarking(binaryData_x, accelerometer_x, BITS, embeddingComplete);

                float result_watermarking_y = DoubleWatermarking(binaryData_y, accelerometer_y, BITS, embeddingComplete);

                float result_watermarking_z = DoubleWatermarking(binaryData_z, accelerometer_z, BITS, embeddingComplete);
            ...
            //processingSum = processingSum + processing_time;

            //Log.d("SENSORS10", "processing time = "+ processing_time);
    ...
    public static String Encryption(double Data) { //binaryData for encryption

        String encrypted_binary_string="";

        byte[] input_string = ByteBuffer.allocate(8).putDouble(Data).array();
        ...
        try {
            ...
            else if(encrypted_bits_counter == 64){ //after 64, next float to do
            ...
            encrypted_bits_counter += 64;

            if(encrypted_bits_counter == 128) //128
            {
